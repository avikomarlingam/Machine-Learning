...
def create_dense_net(nb_classes, img_dim, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=16, dropout_rate=None,
	...
	train_data = train_datagen.flow_from_directory('D:\Mimic_Data\train\AP')
	
	test_datagen = ImageDataGenerator()
	
	test_data = test_datagen.flow_from_directory('D:\Mimic_Data\test\AP')
	
	densenet.compile(optimizer='Adam', loss='mean_squared_error', metrics=['accuracy'], loss_weights=None, sample_weight_mode=None, weighted_metrics=None)
	
	densenet.fit_generator(train_data, steps_per_epoch=2000, epochs=50, validation_data=test_data, validation_steps=800)
...
create_dense_net(nb_classes=num_classes, img_dim=image_dim) '''
